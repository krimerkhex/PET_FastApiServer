...
async def create_async_task(task_id: list[str]):
    ...
    for i, task in enumerate(asnc_task):
        await task
        tasks[task_id[i]].status = "ready"


@Loger
async def crowlering(uid: str):
    client_redis = await create_redis()
    cache = int(await cashing_redis(client_redis, tasks[uid].url))
    if cache is None:
        ...
        tasks[uid].result_code = cache

@Loger
async def cashing_redis(client_redis, url):
    try:
        cache = await client_redis.get(url)
        if cache is None:
            await client_redis.set(url, 1)
        else:
            await client_redis.set(url, int(cache) + 1)
        return await client_redis.get(url)
    except Exception as e:
        logger.error("Exception in function cashing_redis")
